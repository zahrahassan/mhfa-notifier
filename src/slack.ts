...
app.command(
  "/mhfa", async ({ logger, command, ack, respond, }) => {
    ack();
    try {
      const slackHookResponse = await fetch(slackHookUrl, {
        ...
        body: JSON.stringify({ text: `We have received a new message from ${command.user_name}` })
      })
      ...
      if (slackHookResponse.ok) {
        ...
        throw Error(`Slack Hook response was not 200: ${slackHookResponse.status}`)
      }
    } catch (e) {
      logger.error(e);
      await respond({
